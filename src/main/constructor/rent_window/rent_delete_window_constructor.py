...
from src.controllers.rent_controllers.rent_delete_controller import RentDeleteController

class RentDeleteWindowConstructor():
    def __init__(self, right_frame):
        self.right_frame = right_frame
        self.delete_window = RentDeleteWindow(self.right_frame)
        self.read_controller = RentReadController()
        self.delete_controller = RentDeleteController()
    ...
    def read_button_command(self):
        self.delete_window.send_bt.configure(command=self.reading_rent)
    ...
    def deleting_rent(self):
        try:
            message = self.delete_controller.deleting_rent(self.code)
    ...
    def alertWindowCall(self):
        ...
        self.alert_window.yes_bt.configure(command=lambda: [self.deleting_rent(), self.close_alert_window(), self.clean_code_entry(), self.desable_delete_bt()])
