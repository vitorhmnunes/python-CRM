...
class RentDeleteController():
    def __init__(self):
        self.db_operations = DbCRUDOperations()
        self.validator = RentValidators()

    def delete_db_record(self, rent_code):
        try:
            message = self.db_operations.delete_record(table_name='rent', condition_column='code', condition_value=rent_code)
            return message
    ...
    def deleting_rent(self, rent_code):
        try:
            ...
            try: 
                message = self.delete_db_record(rent_code)
                return message
            except Exception as err:
                raise err