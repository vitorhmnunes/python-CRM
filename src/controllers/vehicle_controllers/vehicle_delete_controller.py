...
class VehicleDeleteController():
    ...
    def delete_db_record(self, code_value):
        try:
            message = self.db_operations.delete_record(table_name='vehicle', condition_column='code', condition_value=code_value)
            return message
    ...
    def deleting_vehicle(self, code_value):
        try:
            ...
            try: 
                message = self.delete_db_record(code_value)
                return message
            except Exception as err:
                raise err